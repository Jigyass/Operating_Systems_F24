...
int main() {
  ...
  for (; uninitialized_variable < 100; uninitialized_variable++) {
    ...
    *definitely_lost = (void*) malloc(7); // Give the pointer something else to
    ...
    // Fix: on each iteration, free *definitely_lost, then free definitely_lost
  ...
  // Fix: free still_reachable before program goes out of scope
  free(still_reachable);
  
  // Fix: point the 'possibly_lost' pointer back to start of allocated block, then free
